				...
				<div class="row-fluid">
					<div class="span8">
						<?php
							...
							if($conn)
							{
								$queryu = "SELECT * FROM CLIENTE WHERE USUARIO ='$user_logged'";
								$stidu = oci_parse($conn, $queryu);
								oci_execute($stidu);
								$rowu = oci_fetch_array($stidu, OCI_BOTH);
								$id_cliente = $rowu['ID_CLIENTE'];
									
								//recorrer compra
									$query = oci_parse($conn, "SELECT * FROM COMPRA WHERE ID_CLIENTE = '$id_cliente'");
									oci_execute($query);
